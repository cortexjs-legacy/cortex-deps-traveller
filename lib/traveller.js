...
function Traveller(cache_root, built_root, options) {
  if (typeof cache_root != 'string' || typeof built_root != 'string') {
    return new Error("Must provide cache_root and built_root");
  }
  ...
  if (options.hasOwnProperty('stableOnly'))
    this.stableOnly = options.stableOnly !== false;
  else if (options.hasOwnProperty('stable_only'))
    this.stableOnly = options.stable_only !== false;
...
Traveller.prototype.topsort = require('./sort');
...
Traveller.prototype.__visit = function(child, parent, enter, leave, done, depth) {
  ...
  var dependencies = sortedObject(pkgDeps.bind(this)(pkg));
...
Traveller.prototype.resolvePackage = function(name, range, callback) {
  ...
  self.resolveRange(name, range, function(err, version) {
    ...
    if (self.pkgCache[key]) {
      ...
      readjson.enhanced(depDir, function(err, dep) {
        if (err) return callback(err);
