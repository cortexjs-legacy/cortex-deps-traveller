...
Traveller.prototype.resolveRange = function(name, range, callback) {
  ...
  (function(cb) {
    if (self.verCache[name]) {
      ...
      readVersions.call(self, name, function(err, versions) {
        cb(null, versions);
      });
    }
...
function readVersionsSync(name) {
  var verCache = this.verCache || {};
  // no multi process
  if (verCache[name]) {
    return verCache[name];
  } else {
    var pkg_root = path.join(this.cache_root, name);
    var exists = fs.existsSync(pkg_root);
    if (!exists)
      throw new Error({
        message: "No pacakge '" + name + "' installed, please run 'cortex install' first"
      });

    var vers = fs.readdirSync(pkg_root).filter(semver.valid);
